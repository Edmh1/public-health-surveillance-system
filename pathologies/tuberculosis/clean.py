"""Limpieza especifica de tuberculosis: Pipeline de scikit-learn con ColumnTransformer.

Reproduce las 7 transformaciones en orden: descarte de columnas, minusculas,
conversion de tipos, edad_anios, validacion geografica, enriquecimiento por joins
y consolidacion de grupo etnico.

Los tres codigos SIVIGILA de TB son: 810 (extrapulmonar), 820 (pulmonar) y
825 (farmacoresistente). Los tres se procesan con el mismo pipeline.
"""
...
# Paso 2: columnas de texto que se estandarizan a minusculas.
COLUMNAS_TEXTO_A_MINUSCULAS = ["sexo", "tip_ss", "nom_upgd", "cbmte", "nom_grupo", "cod_ase", "estrato"]

# Paso 3: columnas de fecha a convertir a datetime.
COLUMNAS_FECHA = ["fec_not", "fec_con", "ini_sin", "fec_hos", "fec_def"]

# Paso 3: columnas numericas a convertir a Int64.
COLUMNAS_ENTERAS = [
    "cod_eve",
    "semana",
    "ano",
...
class DescartarColumnasSinValorAnalitico(BaseEstimator, TransformerMixin):
    """Paso 1: descarta columnas sin valor analitico y las redundantes con su codigo."""
...
class ConvertirTipos(BaseEstimator, TransformerMixin):
    """Paso 3: fechas a datetime, ocupacion a Float64, resto de columnas listadas a Int64."""
...
class EnriquecerConReferencias(BaseEstimator, TransformerMixin):
    """Paso 6: enriquece con DIVIPOLA, paises, ocupaciones y aseguradoras (left joins).

    A diferencia de dengue, TB no tiene CIE-10 en los datos (la columna cbmte esta
    presente pero no se usa como codigo de diagnostico clinico).
    """
...
def construir_pipeline_limpieza(ruta_referencias=DIR_REFERENCIAS):
    """Construye el pipeline de limpieza de TB con los 7 pasos de la spec, en orden."""
    pipeline = Pipeline(
        steps=[
