...
Formulas:
- Incidencia TB = (casos 810+820+825 / poblacion en riesgo) x 100.000
- Mortalidad por TB = (muertes TB / poblacion en riesgo) x 100.000
- Letalidad TB = (muertes TB / total casos 810+820+825) x 100
- % Confirmacion bacteriologica = (confirmados / total casos) x 100
- % TB resistente = (casos 825 / total casos 810+820+825) x 100
...
CODIGOS_TB = {COD_TB_EXTRAPULMONAR, COD_TB_PULMONAR, COD_TB_FARMACORESISTENTE}
...
def calcular_indicadores(datos_filtrados: pd.DataFrame, filtros: dict[str, Any]) -> dict[str, Any]:
    ...
    # Mortalidad: casos con fecha de defuncion no nula
    ...
    pct_confirmados = None
    if total_casos and "confirmados" in casos.columns:
        n_confirmados = int((casos["confirmados"] == 1).sum())
        pct_confirmados = n_confirmados / total_casos * 100
